...
export async function getStaticProps(context) {
	console.log(process.env.FS_API_ENDPOINT_ALL);
...
const columns = [
	...
	},
	{
		...
		cell: (row) => (
			<>
				<span className={'role-sm role-' + row.role}>{row.role}</span>
			</>
		),
	},
	{
...
export default function StatsGiocatori(props) {
	...
	const getSearchFilter = () => {
		...
		let roleFilter = document.querySelectorAll(
			'.roles-filter a.tab-active'
		)[0].dataset.role;
		let presenceFilter = document.querySelectorAll(
			'.presence-filter a.tab-active'
		)[0].dataset.percentile;
	...
	const filterData = (args) => {
		...
		if (args.presence != '0%') {
			...
			if (args.presence == '50%') percentileMatch = 38 * 0.5;
	...
	return (
		<div>
			<Head>
				<title>
					Statistiche Giocatori Fantacalcio | {commonData.SiteName}
				</title>
				<meta
					name="description"
					content="Analisi di dati statistici dei giocatori di Serie A negli ultimi anni"
				/>
					...
					<div className="overflow-x-auto">
						<div className="filters mb-4">
							<div className="tabs roles-filter justify-center mb-2">
								<a
									className="tab tab-active tab-bordered"
									data-role="ALL"
									onClick={() => {
										handleRole('ALL');
									}}
							...
							<div className="search-filter mt-4 md:px-10 md:px-0 text-center">
								<input
									id="searchField"
									type="text"
									placeholder="Quale giocatore stai cercando?"
